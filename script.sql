DROP DATABASE IF EXISTS koalla;

CREATE DATABASE IF NOT EXISTS koalla;

USE koalla;

CREATE TABLE IF NOT EXISTS login(
    id INT NOT NULL PRIMARY KEY AUTO_INCREMENT,
    email VARCHAR(100) NULL,
	nome VARCHAR(200) NULL,
	prontuario VARCHAR(8) NULL,
    senha VARCHAR(24) NULL,
	permissao ENUM('professor','funcionario','admin') DEFAULT 'professor',
	created_at TIMESTAMP DEFAULT '1999-10-21 23:30:00',
	updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
	active BOOLEAN DEFAULT TRUE
);

CREATE TABLE IF NOT EXISTS departamento(
	id INT NOT NULL PRIMARY KEY AUTO_INCREMENT,
	nome VARCHAR(255) NULL,
	acronimo VARCHAR(10) NULL,
	created_at TIMESTAMP DEFAULT '1999-10-21 23:30:00',
	updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
	active BOOLEAN DEFAULT TRUE
);

CREATE TABLE IF NOT EXISTS professor (
	id INT NOT NULL PRIMARY KEY AUTO_INCREMENT,
	id_departamento INT NOT NULL,
	id_login INT NOT NULL,
    siape INT(8) NULL,
	telefone VARCHAR(20) NULL,
	created_at TIMESTAMP DEFAULT '1999-10-21 23:30:00',
	updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
	active BOOLEAN DEFAULT TRUE,
	CONSTRAINT fk_prof_dep FOREIGN KEY (id_departamento)
    REFERENCES departamento(id),
    CONSTRAINT fk_prof FOREIGN KEY (id_login)
    REFERENCES login(id)
);

CREATE TABLE IF NOT EXISTS curso(
	id INT NOT NULL PRIMARY KEY AUTO_INCREMENT,
	nome VARCHAR(255) NULL,
	semestral BOOLEAN NULL,
	id_departamento INT NOT NULL,
	created_at TIMESTAMP DEFAULT '1999-10-21 23:30:00',
	updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
	active BOOLEAN DEFAULT TRUE,
	CONSTRAINT fk_curso_dep FOREIGN KEY (id_departamento)
	REFERENCES departamento(id) ON DELETE CASCADE
);

CREATE TABLE IF NOT EXISTS disciplina(
	id INT NOT NULL PRIMARY KEY AUTO_INCREMENT,
	nome VARCHAR(255) NULL,
	acronimo VARCHAR(10) NULL,
	id_curso INT NOT NULL,
	CONSTRAINT fk_discp_curso FOREIGN KEY (id_curso)
	REFERENCES curso(id) ON DELETE CASCADE,
	created_at TIMESTAMP DEFAULT '1999-10-21 23:30:00',
	updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
	active BOOLEAN DEFAULT TRUE
);

CREATE TABLE IF NOT EXISTS reserva(
	id INT NOT NULL PRIMARY KEY AUTO_INCREMENT,
	tipo ENUM('avulsa', 'fixa', 'especial') DEFAULT 'avulsa',
	id_login INT NOT NULL,
	lab INT(4) NULL,
	justificativa VARCHAR(512) NULL,
    `status` ENUM('andamento', 'aprovado', 'reprovado', 'fechado') DEFAULT 'andamento',
	CONSTRAINT fk_rsv_login FOREIGN KEY (id_login)
	REFERENCES login(id),
	created_at TIMESTAMP DEFAULT '1999-10-21 23:30:00',
	updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
	active BOOLEAN DEFAULT TRUE
);

CREATE TABLE IF NOT EXISTS horario(
	id INT NOT NULL PRIMARY KEY AUTO_INCREMENT,
	dt_inicio TIMESTAMP DEFAULT '1999-10-21 23:30:00',
	dt_fim TIMESTAMP DEFAULT '1999-10-21 23:30:00',
	sigla VARCHAR(2) NULL,
	created_at TIMESTAMP DEFAULT '1999-10-21 23:30:00',
	updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
	active BOOLEAN DEFAULT TRUE
);

CREATE TABLE IF NOT EXISTS horario_reserva(
	id INT NOT NULL PRIMARY KEY AUTO_INCREMENT,
	id_horario INT NOT NULL,
	id_reserva INT NOT NULL,
	dia TIMESTAMP DEFAULT '1999-10-21 23:30:00',
	CONSTRAINT fk_hr_rsv FOREIGN KEY (id_horario)
	REFERENCES horario(id),
	CONSTRAINT fk_rsv_hr FOREIGN KEY (id_reserva)
	REFERENCES reserva(id),
	created_at TIMESTAMP DEFAULT '1999-10-21 23:30:00',
	updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
	active BOOLEAN DEFAULT TRUE
);

CREATE TABLE IF NOT EXISTS avaliacao(
    id INT NOT NULL PRIMARY KEY AUTO_INCREMENT,
	problema BOOLEAN DEFAULT FALSE,
	tipo ENUM('ocorrencia', 'anormalidade'),
	descricao VARCHAR(1024) NULL,
	id_reserva INT NOT NULL,
	CONSTRAINT fk_aval_rsv FOREIGN KEY (id_reserva)
	REFERENCES horario_reserva(id),
	created_at TIMESTAMP DEFAULT '1999-10-21 23:30:00',
	updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
	active BOOLEAN DEFAULT TRUE
); 

CREATE TABLE IF NOT EXISTS calendario(
	id INT NOT NULL PRIMARY KEY AUTO_INCREMENT,
	tipo ENUM('1', '2', '3') NULL,
	dt_inicio TIMESTAMP DEFAULT '1999-10-21 23:30:00',
	dt_fim TIMESTAMP DEFAULT '1999-10-21 23:30:00',
	created_at TIMESTAMP DEFAULT '1999-10-21 23:30:00',
	updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
	active BOOLEAN DEFAULT TRUE
);

INSERT INTO departamento(nome) VALUES('glayson');

INSERT INTO horario(sigla) VALUES('M1'),('M2'),('M3'),('M4'),('M5'),('M6'),
('V1'),('V2'),('V3'),('V4'),('V5'),('V6'),
('N1'),('N2'),('N3'),('N4'),('N5');

INSERT INTO login(permissao, prontuario, senha) VALUES('admin', '1651153', 'admin'), 
('funcionario', '1650084', 'admin');